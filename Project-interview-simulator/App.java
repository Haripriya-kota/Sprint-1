...
public class App {
	public static void main(String[] args) {
		...
		while (true) {
			...
			switch (choice) {
			case 1:
				// Option 1: Call createAccount method in InterviewService to handle account
				// creation
				interviewService.createAccount(scanner);
				break;
			case 2:
				// Option 2: Call scheduleInterviewSession method in InterviewService to handle
				// interview scheduling
				interviewService.scheduleInterviewSession(scanner);
				break;
			case 3:
				// Option 3: Start an instant interview by asking the user for their email and
				// passing it to the service
				System.out.print("Enter your Email: ");
				String email = scanner.nextLine();
				try {
                    interviewService.startInstantInterview(email, scanner); // This may throw InterruptedException
                } catch (InterruptedException e) {
                    System.out.println("❌ Interview interrupted. Please try again.");
	...
	/*
	 * Helper method to get a valid integer input from the user. This method ensures
	 * that the user enters a valid number.
	 * 
